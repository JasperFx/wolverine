...
`GroupBy(IGroupingRule)`. Those rules apply only to the messages published through that topology, and they are
authoritative: once a topology declares a rule, the application-wide `MessagePartitioning` rules are not consulted
for its messages at all, even when none of the topology's own rules match. That makes the grouping independent of
...
```
<sup><a href='https://github.com/JasperFx/wolverine/blob/main/src/Samples/DocumentationSamples/PartitioningSamples.cs#L76-L96' title='Snippet source file'>snippet source</a> | <a href='#snippet-sample_topology_scoped_grouping' title='Start of snippet'>anchor</a></sup>
<!-- endSnippet -->
...
at most one of those topologies may declare grouping rules for it. Wolverine checks this for every handled message
