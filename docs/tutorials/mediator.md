...
* Sql Server as the backing database store, using [Wolverine's Sql Server message persistence](../guide/durability/index.md#using-sql-server-for-message-storage)
...
into Wolverine's [extended command line support](../guide/command-line.md):
...
executes the correct Wolverine handler(s) as well as any of the registered [Wolverine middleware](../guide/handlers/middleware.md).

::: tip
This execution happens inline, but will use the *RetryNow* error handling capabilities. See [Wolverine's error handling](../guide/handlers/error-handling.md) for more information.
:::

See also:

* [Cascading messages from actions](../guide/handlers/cascading.md) for a better explanation of how the `ItemCreated`
  event message is automatically published if the handler success.
* [Messages](../guide/messages.md) for the details of messages themselves including versioning, serialization, and forwarding.
* [Message handlers](../guide/handlers/index.md) for the details of how to write Wolverine message handlers and how they
  are discovered
