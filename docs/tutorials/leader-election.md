...
* **How does the leadership election work?** Through the message store, using whatever global, advisory-lock mechanism the current persistence provides (a session-scoped advisory lock in PostgreSQL and SQL Server). There is no node-to-node consensus and no bully-style messaging: on every health-check tick, each node simply *tries to attain the leadership lock*. Whoever wins the lock writes the `wolverine://leader` assignment to itself and becomes the leader; everyone else stays a follower and tries again next tick. A leader that discovers server-side that it no longer holds the lock — for example after its database session was dropped — steps down cleanly on that same tick and the normal election takes over. Because the lock is session-scoped, a leader that dies or is partitioned away from the database has its lock released automatically, so a node that can still reach the database can take over.

  ::: tip
  The election contract is modeled and machine-checked with the [P model checker](https://github.com/p-org/P) under [`formal/leader-election/`](https://github.com/JasperFx/wolverine/tree/main/formal/leader-election) — crashes, graceful stops, dropped lock sessions, heartbeat blips, and network partitions, all checked against a "converge to exactly one leader" property.
  :::

* **What happens during a network partition?** Leadership is **CP**: it favors consistency over availability. Since everything routes through the shared message store, the store is the arbiter. Nodes that can still reach the database keep heartbeating and, if the old leader was stranded on the far side, elect a new leader once its session-scoped lock is released. A node that is cut off from the database cannot renew its lock or write its heartbeat: it steps down if it was the leader, elects no one, and stops doing leadership and agent-assignment work until it can reach the store again — at which point it re-registers and rejoins. In other words, a partition never produces two simultaneous leaders; the minority side simply stops rather than splitting the brain.
