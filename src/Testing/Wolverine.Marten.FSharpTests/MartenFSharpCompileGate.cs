...
/// <summary>
///     The Marten document acceptance gate (issue GH-2969): regenerates the fixture's
///     <c>Generated.fs</c> from the sample's Marten document handler chain, then shells
///     <c>dotnet build</c> on the checked-in F# fixture and asserts a clean build. Mirrors the
///     Core/Http/EfCore surface gates.
/// </summary>
public class MartenFSharpCompileGate
{
    private readonly ITestOutputHelper _output;

    public MartenFSharpCompileGate(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    {
        var code = MartenFSharpCodegenSample.GenerateCode();
        var generatedFile = MartenFSharpCodegenSample.DefaultGeneratedFilePath();
        File.WriteAllText(generatedFile, code);

        File.Exists(generatedFile).ShouldBeTrue();
        _output.WriteLine(code);

        var fixtureProject = MartenFSharpCodegenSample.FixtureProjectPath();
