...
/// <summary>
///     The Marten aggregate acceptance gate (issue GH-2969): regenerates the fixture's
///     <c>Generated.fs</c> from the sample's event-sourced aggregate handler chain, then shells
///     <c>dotnet build</c> on the checked-in F# fixture and asserts a clean build. Mirrors the
///     Core/Http/EfCore/Marten surface gates.
/// </summary>
public class MartenAggregateFSharpCompileGate
{
    private readonly ITestOutputHelper _output;

    public MartenAggregateFSharpCompileGate(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    {
        var code = MartenAggregateFSharpCodegenSample.GenerateCode();
        var generatedFile = MartenAggregateFSharpCodegenSample.DefaultGeneratedFilePath();
        File.WriteAllText(generatedFile, code);

        File.Exists(generatedFile).ShouldBeTrue();
        _output.WriteLine(code);

        var fixtureProject = MartenAggregateFSharpCodegenSample.FixtureProjectPath();
