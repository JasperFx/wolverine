...
public static class RabbitMqTransportExtensions
{
    ...
    /// <returns></returns>
    internal static RabbitMqTransport RabbitMqTransport(this WolverineOptions endpoints, BrokerName? name = null)
    {
        var transports = endpoints.As<WolverineOptions>().Transports;

        return transports.GetOrCreate<RabbitMqTransport>(name);
    }
    ...
    /// <param name="name">If connecting to more than one Rabbit MQ broker, this is the identity of the additional broker</param>
    ...
    public static RabbitMqExchangeConfiguration PublishMessagesToRabbitMqExchange<T>(this WolverineOptions options, string exchangeName,
        Func<T, string> topicSource, BrokerName? name = null)
    {
        var transport = options.RabbitMqTransport(name);
    ...
    /// <summary>
    /// Configure connection and authentication information for a secondary Rabbit MQ broker
    /// to this application. Only use this overload if your Wolverine application needs to talk
    /// to two or more Rabbit MQ brokers
    /// </summary>
    /// <param name="options"></param>
    /// <param name="name">Name of the additional Rabbit Mq broker</param>
    /// <param name="configure"></param>
    public static RabbitMqTransportExpression AddNamedRabbitMqBroker(this WolverineOptions options, BrokerName name,
        Action<ConnectionFactory> configure)
    {
        var transport = options.RabbitMqTransport(name);
    ...
    ///     Optional configuration for this Rabbit Mq queue if being initialized by Wolverine
    ///     <returns></returns>
    public static RabbitMqListenerConfiguration ListenToRabbitQueueOnNamedBroker(this WolverineOptions endpoints, BrokerName name, string queueName,
        Action<IRabbitMqQueue>? configure = null)
    {
        var transport = endpoints.RabbitMqTransport(name);
        var corrected = transport.MaybeCorrectName(queueName);
        var queue = transport.Queues[corrected];
        queue.EndpointName = queueName;
        queue.IsListener = true;
    ...
    public static RabbitMqSubscriberConfiguration ToRabbitTopic(this IPublishToExpression publishing,
        string topicName, string exchangeName, BrokerName? name = null)
    {
        var transports = publishing.As<PublishingExpression>().Parent.Transports;
        var transport = transports.GetOrCreate<RabbitMqTransport>(name);
    ...
    /// </summary>
    /// <param name="publishing"></param>
    /// <param name="name">Name of the Rabbit MQ broker
    /// <param name="routingKeyOrQueue">
    ///     This is used as the routing key when publishing. Can be either a binding key or a queue
    ///     name or a static topic name if the exchange is topic-based
    /// </param>
    /// <returns></returns>
    public static RabbitMqSubscriberConfiguration ToRabbitQueueOnNamedBroker(this IPublishToExpression publishing, BrokerName name,
        string queueName, Action<IRabbitMqQueue>? configure = null)
    {
        var transports = publishing.As<PublishingExpression>().Parent.Transports;
        var transport = transports.GetOrCreate<RabbitMqTransport>(name);
    ...
    /// <param name="configure">Optional configuration of this exchange if Wolverine is doing the initialization in Rabbit MQ</param>
    /// <returns></returns>
    public static RabbitMqSubscriberConfiguration ToRabbitExchangeOnNamedBroker(this IPublishToExpression publishing, BrokerName name,
        string exchangeName, Action<IRabbitMqBindableExchange>? configure = null)
    {
        var transports = publishing.As<PublishingExpression>().Parent.Transports;
        var transport = transports.GetOrCreate<RabbitMqTransport>(name);
    ...
    /// <param name="configure"></param>
    /// <returns></returns>
    public static RabbitMqSubscriberConfiguration ToRabbitRoutingKeyOnNamedBroker(this IPublishToExpression publishing, BrokerName name, string exchangeName, string routingKey,
        Action<IRabbitMqExchange>? configure = null)
    {
        var transports = publishing.As<PublishingExpression>().Parent.Transports;
        var transport = transports.GetOrCreate<RabbitMqTransport>(name);
    ...
    /// <param name="configure">Optional configuration of this exchange if Wolverine is doing the initialization in Rabbit MQ</param>
    /// <returns></returns>
    public static RabbitMqSubscriberConfiguration ToRabbitTopicsOnNamedBroker(this IPublishToExpression publishing, BrokerName name,
        string exchangeName, Action<IRabbitMqBindableExchange>? configure = null)
    {
        var transports = publishing.As<PublishingExpression>().Parent.Transports;
        var transport = transports.GetOrCreate<RabbitMqTransport>(name);
    ...
    /// <summary>
    /// Use sharded Rabbit MQ queues for global partitioned message processing.
    ...
    /// <param name="configure"></param>
    /// <returns></returns>
    public static GlobalPartitionedMessageTopology UseShardedRabbitQueues(
