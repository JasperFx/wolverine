...
public class AmazonSqsQueue : AmazonEndpoint
{
    ...
    internal AmazonSqsQueue(string queueName, AmazonSqsTransport parent) 
        : base(queueName, parent, new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.QueueSegment}/{queueName}"))
    {
        QueueName = queueName;
    ...
    public override async ValueTask<Dictionary<string, string>> GetAttributesAsync()
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {
                nameof(GetQueueAttributesResponse.QueueARN),
                atts.QueueARN
            }
        };
    }
    ...
    private async Task SetupAsync(IAmazonSQS client)
    {
    ...
    private async Task PurgeAsync(IAmazonSQS client)
    {
        await LoadQueueUrlIfEmpty(client);
