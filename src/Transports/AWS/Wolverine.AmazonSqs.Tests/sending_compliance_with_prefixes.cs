...
public class PrefixedComplianceFixture : TransportComplianceFixture, IAsyncLifetime
{
    ...
    public PrefixedComplianceFixture() : base(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.SqsSegment}/foo-buffered-receiver"), 120)
    {
    }
    ...
    {
        var number = ++Number;
        OutboundAddress = new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.SqsSegment}/foo-prefix-receiver-" + number);
