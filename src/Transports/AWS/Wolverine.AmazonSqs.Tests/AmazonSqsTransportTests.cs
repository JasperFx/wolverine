...
public class AmazonSqsTransportTests
{
    ...
    public void findEndpointByUri_should_correctly_find_by_queueName()
    {
        ...
        var result = transport.GetOrCreateEndpoint(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.QueueSegment}/{queueNameInPascalCase}"));
    ...
    public void findEndpointByUri_should_correctly_find_by_topicName()
    {
        ...
        var result = transport.GetOrCreateEndpoint(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.QueueSegment}/{topicNameInPascalCase}"));
    ...
    public void findEndpointByUri_should_correctly_create_queue_if_it_doesnt_exist()
    {
        ...
        var result = transport.GetOrCreateEndpoint(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.QueueSegment}/{queueName}"));
        transport.Queues.Count.ShouldBe(1);

        result.EndpointName.ShouldBe(queueName);

        var result2 = transport.GetOrCreateEndpoint(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.QueueSegment}/{queueName2}"));
        transport.Queues.Count.ShouldBe(2);
    ...
    public void findEndpointByUri_should_correctly_create_topic_if_it_doesnt_exist()
    {
        ...
        var result = transport.GetOrCreateEndpoint(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.TopicSegment}/{topicName}"));
        transport.Topics.Count.ShouldBe(1);

        result.EndpointName.ShouldBe(topicName);

        var result2 = transport.GetOrCreateEndpoint(new Uri($"{AmazonSqsTransport.SqsProtocol}://{AmazonSqsTransport.TopicSegment}/{topicName2}"));
        transport.Topics.Count.ShouldBe(2);
