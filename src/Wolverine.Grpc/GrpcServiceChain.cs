...
///     concrete subclass. Each virtual RPC method on the proto-generated <c>*Base</c> class is
...
public class GrpcServiceChain : Chain<GrpcServiceChain, ModifyGrpcServiceChainAttribute>, ICodeFile
{
    ...
    public GrpcServiceChain(Type stubType, GrpcGraph parent)
    {
        ...
        ServerStreamingMethods = SupportedMethods
            ...
            .Select(m => m.Method)
            .ToArray();
        ClientStreamingMethods = SupportedMethods
            .Where(m => m.Kind == GrpcMethodKind.ClientStreaming)
            .Select(m => m.Method)
            .ToArray();
    ...
    void ICodeFile.AssembleTypes(GeneratedAssembly assembly)
    {
        ...
        // Wolverine.Grpc — home of WolverineGrpcStreamAdapters.ReadAllAsync, used by the
        // client-streaming forward frame to adapt IAsyncStreamReader<T> to IAsyncEnumerable<T>
        assembly.ReferenceAssembly(typeof(WolverineGrpcStreamAdapters).Assembly);
        ...
        foreach (var rpc in SupportedMethods)
        {
            ...
            // in scope when the method begins. Bidi and client-streaming methods start with an
            // IAsyncStreamReader<T> rather than a single T — per-call middleware is not woven for them.
            if (rpc.Kind is not (GrpcMethodKind.BidirectionalStreaming or GrpcMethodKind.ClientStreaming))
            {
            ...
            switch (rpc.Kind)
            {
                ...
                case GrpcMethodKind.ClientStreaming:
                    generatedMethod.AsyncMode = AsyncMode.AsyncTask;
                    generatedMethod.Frames.Add(new ForwardClientStreamToMessageBusFrame(rpc.Method, busField));
                    break;
    ...
    ///     All four canonical gRPC shapes (unary, server-streaming, client-streaming, bidirectional) are
    ///     code-generated; methods matching none of them (e.g., user-added non-RPC virtuals) are skipped.
...
///     </code>
///     Before-frames (including the Validate short-circuit) are not woven into client-streaming
///     methods for the same reason as bidirectional ones: they require a concrete <c>TRequest</c>
///     in scope before the stream is consumed, which the signature does not provide.
/// </remarks>
internal sealed class ForwardClientStreamToMessageBusFrame : AsyncFrame
{
    private readonly MethodInfo _rpc;
    private readonly InjectedField _busField;

    public ForwardClientStreamToMessageBusFrame(MethodInfo rpc, InjectedField busField)
    {
        _rpc = rpc;
        _busField = busField;
    }
    ...
    public override void GenerateCode(GeneratedMethod method, ISourceWriter writer)
    {
        var parameters = _rpc.GetParameters();
        var readerName = ForwardUnaryToMessageBusFrame.ParameterName(parameters, 0);
        var contextName = ForwardUnaryToMessageBusFrame.ParameterName(parameters, 1);

        // IAsyncStreamReader<TRequest> → TRequest; Task<TResponse> → TResponse
        ...
        if (Next == null)
        {
            writer.Write($"return await {busInvoke};");
        }
        else
        {
            writer.Write($"var result = await {busInvoke};");
