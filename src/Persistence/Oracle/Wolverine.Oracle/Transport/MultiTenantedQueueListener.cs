...
public class MultiTenantedQueueListener : IListener
{
    ...
    private ImHashMap<string, OracleQueueListener> _listeners = ImHashMap<string, OracleQueueListener>.Empty;
    private Task? _activator;

    public MultiTenantedQueueListener(ILogger logger, OracleQueue queue, MultiTenantedMessageStore stores, IWolverineRuntime runtime, IReceiver receiver)
    {
        _logger = logger;
        _queue = queue;
        _stores = stores;
    ...
    public async Task StartAsync()
    {
        ...
        foreach (var store in _stores.Source.AllActive().OfType<OracleMessageStore>())
        {
        ...
        _activator = Task.Run(async () =>
        {
            while (!_cancellation.IsCancellationRequested)
            {
                ...
                foreach (var store in databases.OfType<OracleMessageStore>())
                {
    ...
    private async Task startListening(OracleMessageStore store)
    {
        var listener = new OracleQueueListener(_queue, _runtime, _receiver, store.OracleDataSource, store.Name);
        _listeners = _listeners.AddOrUpdate(store.Name, listener);
        await listener.StartAsync();

        _logger.LogInformation("Started message listening for Oracle queue {QueueName} on database {Database}", _queue.Name, store.Name);
    }
