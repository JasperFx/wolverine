...
internal class NpgsqlConnectionInstance : Instance
{
    ...
    {
        return new NpgsqlConnection(scope.GetInstance<DatabaseSettings>().ConnectionString);
    }
...
public class NpgsqlConnectionFrame : SyncFrame
{
    ...
    {
        writer.Write(
            $"BLOCK:using ({_instance.ServiceType.FullNameInCode()} {Connection.Usage} = new {typeof(NpgsqlConnection).FullName}({_settings.Usage}.{nameof(DatabaseSettings.ConnectionString)}))");
