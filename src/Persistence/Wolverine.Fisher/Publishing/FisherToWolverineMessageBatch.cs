...
/// <summary>
///     Fisher side of the projection-message bridge to Wolverine. One instance
...
///     <list type="bullet">
///         <item><description>
///             Fisher's <see cref="IMessageBatch.BeforeCommitAsync"/> /
///             <see cref="IMessageBatch.AfterCommitAsync"/> take just a
///             <see cref="CancellationToken"/> — Fisher already owns the
...
///             the enlisted <see cref="FisherEnvelopeTransaction"/>). Kept on the
...
internal class FisherToWolverineMessageBatch(MessageContext Context, IDocumentSession Session) : IMessageBatch
#pragma warning restore CS9113
{
    ...
    public Task BeforeCommitAsync(CancellationToken token)
    {
        ...
        // Marten bridge does the same. Fisher surfaces a pre-commit hook for
