...
/// <summary>
/// Marker interface for declarative data requirement checks against a Fisher document store.
...
/// <see cref="IFisherDataRequirement"/> values participate on the same chain alongside other
/// batchable operations (e.g., <c>[ReadAggregate]</c>, <c>[WriteAggregate]</c>), Wolverine
/// combines their queries into a single Fisher <see cref="IBatchedQuery"/> round-trip.
/// </summary>
public interface IFisherDataRequirement
{
    ...
    /// <summary>Enlist the requirement's existence check in a Fisher batch query.</summary>
...
/// designated Fisher document exists. Throws <see cref="RequiredDataMissingException"/> when
/// the document is missing.
/// </summary>
public class DocumentExists<TDoc, TId> : IFisherDataRequirement where TDoc : class
{
    ...
    public async Task CheckAsync(IDocumentSession session, ILogger logger, CancellationToken cancellation)
    {
        ...
        if (!exists)
        {
            logger.LogWarning("Fisher data requirement failure: {Message}", _missingMessage);
...
#pragma warning restore VSTHRD003 // Avoid awaiting foreign Tasks
    ...
    private static string UnsupportedIdMessage() =>
        $"Fisher does not support id type {typeof(TId).FullNameInCode()} for {typeof(TDoc).NameInCode()}. " +
        "Supported id types are System.Guid and System.String - Fisher identities are Guid or string, " +
        "with no numeric or HiLo identity.";
}

/// <summary>
/// Returning this from a "Before" / "Validate" method opts into a declarative check that the
/// designated Fisher document does NOT exist. Throws <see cref="RequiredDataMissingException"/>
/// when the document already exists.
/// </summary>
public class DocumentDoesNotExist<TDoc, TId> : IFisherDataRequirement where TDoc : class
{
    private readonly TId _identity;
    ...
    public async Task CheckAsync(IDocumentSession session, ILogger logger, CancellationToken cancellation)
    {
        ...
        if (exists)
        {
            logger.LogWarning("Fisher data requirement failure: {Message}", _existsMessage);
