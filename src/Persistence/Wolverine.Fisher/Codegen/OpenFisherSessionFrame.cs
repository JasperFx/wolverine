...
internal class OpenFisherSessionFrame : AsyncFrame
{
    private readonly Type _sessionType;
    private Variable? _context;
    private Variable? _factory;
    private Variable? _fisherFactory;
    private Variable _tenantId = null!;
    private bool _justCast;

    public OpenFisherSessionFrame(Type sessionType)
    {
    ...
    public override void GenerateCode(GeneratedMethod method, ISourceWriter writer)
    {
        ...
        if (_context == null)
        {
            // Just use native Fisher here.
            writer.Write($"await using var {ReturnVariable.Usage} = {_fisherFactory!.Usage}.{methodName}();");
        }
        else if (_tenantId == null)
        {
            writer.WriteComment("Building the Fisher session");
            writer.Write($"await using var {ReturnVariable.Usage} = {_factory!.Usage}.{methodName}({_context!.Usage});");
        }
        else
        {
            writer.WriteComment("Building the Fisher session using the detected tenant id");
    ...
    public override IEnumerable<Variable> FindVariables(IMethodVariables chain)
    {
        ...
        {
            _fisherFactory = chain.FindVariable(typeof(ISessionFactory));
            yield return _fisherFactory;
        }
    }
}
