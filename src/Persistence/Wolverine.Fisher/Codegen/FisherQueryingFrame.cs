...
internal class FisherBatchingPolicy : IMethodPreCompilationPolicy
{
    public void Apply(IGeneratedMethod method)
    {
        ...
        var batchFrame = new FisherBatchFrame();
...
internal class FisherBatchFrame : AsyncFrame
{
    ...
    {
        writer.WriteLine(
            // Fisher hangs CreateBatchQuery() off the session's event operations rather than the
            // session itself, so the emitted source says session.Events.CreateBatchQuery().
            $"var {BatchQuery.Usage} = {_session.Usage}.Events.CreateBatchQuery();");
