...
/// <summary>
/// Interface for any kind of Fisher related side effect
/// </summary>
public interface IFisherOp : ISideEffect
{
...
internal class FisherOpPolicy : IChainPolicy
{
    public void Apply(IReadOnlyList<IChain> chains, GenerationRules rules, IServiceContainer container)
    {
        ...
        {
            var candidates = chain.ReturnVariablesOfType<IEnumerable<IFisherOp>>().ToArray();
            if (candidates.Any())
            {
            ...
            {
                collection.UseReturnAction(v => new ForEachFisherOpFrame(v));
            }
        }
    }
}

internal class ForEachFisherOpFrame : SyncFrame
{
    private readonly Variable _collection;
    private Variable _session = null!;

    public ForEachFisherOpFrame(Variable collection)
    {
    ...
    {
        writer.WriteComment("Apply each Fisher op to the current document session");
        writer.Write($"foreach (var item_of_{_collection.Usage} in {_collection.Usage}) item_of_{_collection.Usage}.{nameof(IFisherOp.Execute)}({_session.Usage});");
...
public static class FisherOps
{
    ...
    /// <see cref="Wolverine.Fisher.Requirements.IFisherDataRequirement"/> from a "Before" /
    /// "Validate" method on a handler. Multiple data requirements stacked on the same chain
    /// are batched into a single Fisher round-trip.
    /// </summary>
    ...
    /// <summary>
    /// Return a side effect of storing an enumerable of potentially mixed document types in Fisher
    /// </summary>
...
public class StoreObjects : DocumentsOp
{
    ...
    {
        // Fisher does not have a single StoreObjects(IEnumerable<object>) method like Marten,
...
/// <summary>
/// Fluent builder for declaring an <see cref="Wolverine.Fisher.Requirements.IFisherDataRequirement"/>
/// against a Fisher document. Returned from <see cref="FisherOps.Document{TDoc}"/>; pair with
...
public class CheckDocument<TDoc> where TDoc : class
{
    ...
    public Wolverine.Fisher.Requirements.IFisherDataRequirement MustExist<TId>(TId id) =>
        new Wolverine.Fisher.Requirements.DocumentExists<TDoc, TId>(id);

    /// <summary>Require that a document of type <typeparamref name="TDoc"/> with the supplied identity does NOT exist.</summary>
    public Wolverine.Fisher.Requirements.IFisherDataRequirement MustNotExist<TId>(TId id) =>
        new Wolverine.Fisher.Requirements.DocumentDoesNotExist<TDoc, TId>(id);
    ...
    public Wolverine.Fisher.Requirements.IFisherDataRequirement MustExist<TId>(TId id, string missingMessage) =>
        new Wolverine.Fisher.Requirements.DocumentExists<TDoc, TId>(id, missingMessage);
    ...
    public Wolverine.Fisher.Requirements.IFisherDataRequirement MustNotExist<TId>(TId id, string existsMessage) =>
        new Wolverine.Fisher.Requirements.DocumentDoesNotExist<TDoc, TId>(id, existsMessage);
}
