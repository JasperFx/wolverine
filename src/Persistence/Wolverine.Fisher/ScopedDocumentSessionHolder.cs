...
/// <see cref="IServiceScope"/> off the root provider; the Fisher scoping frame primes this holder in
...
/// one (which would defeat the transaction boundary). See GH-3001 and GH-4145.
///
/// The holder is empty in non-handler scopes (hosted services, admin tools, raw resolution), where
/// the decorated registration falls back to Fisher's own session factory.
/// </summary>
public sealed class ScopedDocumentSessionHolder : IScopedSessionHolder<IDocumentSession>
{
