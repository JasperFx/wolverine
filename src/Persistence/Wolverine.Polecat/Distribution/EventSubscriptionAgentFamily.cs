...
public class EventSubscriptionAgentFamily : IStaticAgentFamily, IAsyncDisposable
{
    ...
    public EventSubscriptionAgentFamily(IEnumerable<IEventStore> stores,
        IEnumerable<IObserver<ShardState>> observers)
    {
        _observers = observers.ToArray();
        foreach (var store in stores)
        {
            _stores = _stores.AddOrUpdate(store.Identity.ToString(),
                new EventStoreAgents(store, _observers));
        }
    }
    ...
    public async ValueTask<IAgent> BuildAgentAsync(Uri uri, IWolverineRuntime wolverineRuntime)
    {
        ...
        throw new AgentStartingException(uri, wolverineRuntime.Options.UniqueNodeId,
            new ArgumentOutOfRangeException(nameof(uri),
                "Unknown event projection or subscription"));
    }
