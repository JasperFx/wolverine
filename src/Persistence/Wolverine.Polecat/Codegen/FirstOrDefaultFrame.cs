...
internal class FirstOrDefaultFrame : AsyncFrame
{
    ...
    public override void GenerateCode(GeneratedMethod method, ISourceWriter writer)
    {
        ...
        writer.Write(
            $"var {Result.Usage} = await {typeof(PolecatQueryableExtensions).FullNameInCode()}.{nameof(PolecatQueryableExtensions.FirstOrDefaultAsync)}({_session!.Usage}.{nameof(IQuerySession.Query)}<{_entityType.FullNameInCode()}>(), {_cancellation!.Usage}).ConfigureAwait(false);");
