...
///     Polecat's own derived one, so that Wolverine's store agnostic <c>Storage.AppendEvents()</c> /
///     <c>Storage.StartStream()</c> side effects resolve against any registered event store. The sibling
///     <c>EventOperationsSource</c> above stays as it is because a handler asking for Polecat's own type
