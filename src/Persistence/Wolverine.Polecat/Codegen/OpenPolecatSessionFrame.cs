...
internal class OpenPolecatSessionFrame : AsyncFrame
{
    ...
    public override void GenerateCode(GeneratedMethod method, ISourceWriter writer)
    {
        ...
        if (_context == null)
        {
            // Just use native Polecat here.
            writer.Write($"await using var {ReturnVariable.Usage} = {_polecatFactory!.Usage}.{methodName}();");
        }
        else if (_tenantId == null)
        {
            writer.WriteComment("Building the Polecat session");
            writer.Write($"await using var {ReturnVariable.Usage} = {_factory!.Usage}.{methodName}({_context!.Usage});");
        }
        else
        {
            writer.WriteComment("Building the Polecat session using the detected tenant id");
    ...
    public override IEnumerable<Variable> FindVariables(IMethodVariables chain)
    {
        ...
        {
            _polecatFactory = chain.FindVariable(typeof(ISessionFactory));
            yield return _polecatFactory;
        }
    }
}
