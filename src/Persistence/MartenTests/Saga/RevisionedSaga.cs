...
public class using_revisioned_sagas : IAsyncLifetime
{
    ...
    public async Task execute_using_update_revision()
    {
        ...
        var execution = Task.Run(() => theHost.MessageBus().InvokeAsync(slow));
...
public class RevisionedSaga : Wolverine.Saga, IRevisioned
{
    ...
    public Task Handle(SlowMessage message) => message.Source.Task;
