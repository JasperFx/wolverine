...
public class MultiTenantedQueueListener : IListener
{
    ...
    private ImHashMap<string, SqlServerQueueListener> _listeners = ImHashMap<string, SqlServerQueueListener>.Empty;
    private Task? _activator;

    public MultiTenantedQueueListener(ILogger logger, SqlServerQueue queue, MultiTenantedMessageStore stores,
    ...
    {
        if (_queue.Parent.Storage != null)
        {
            await startListening(_queue.Parent.Storage);
        }

        foreach (var store in _stores.Source.AllActive().OfType<SqlServerMessageStore>())
        {
        ...
        _activator = Task.Run(async () =>
        {
            while (!_cancellation.IsCancellationRequested)
            {
                ...
                foreach (var store in databases.OfType<SqlServerMessageStore>())
                {
    ...
    private async Task startListening(SqlServerMessageStore store)
    {
        ...
        _logger.LogInformation("Started message listening for SQL Server queue {QueueName} on database {Database}",
