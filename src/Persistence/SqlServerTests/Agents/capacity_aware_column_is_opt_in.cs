...
/// <summary>
/// GH-4593, the SQL Server half. Same contract the PostgreSQL store has had since GH-3959: the
/// load_factor column and every statement naming it are gated on CapacityAwareAssignment, so upgrading
/// without opting in migrates nothing and reads nothing.
/// </summary>
public class capacity_aware_column_is_opt_in
{
    ...
    private static Task<IHost> startAsync(string schema, bool capacityAware, AutoCreate autoCreate)
    {
        return Host.CreateDefaultBuilder()
            .UseWolverine(opts =>
            {
                ...
                // Capacity-aware assignment requires an explicit load monitor and the runtime refuses to
                // start without one (GH-4589). These tests are about the COLUMN being gated on the flag,
                // so the reading itself is a constant.
                if (capacityAware)
                {
    ...
    private static async Task<int> loadFactorColumnCountAsync(string schema)
    {
    ...
    {
        const string schema = "capacity_off_4593";
    ...
    {
        const string schema = "capacity_on_4593";
        await dropSchemaAsync(schema);
    ...
    /// <summary>
    /// The case the gate exists for: an AutoCreate.None deployment that upgrades without opting in never
    /// gets the column provisioned, and must not be asked for it.
    /// </summary>
    [Fact]
    public async Task auto_create_none_against_a_database_without_the_column()
    {
        const string schema = "capacity_none_4593";
        await dropSchemaAsync(schema);

        // Provision with the column, then drop it: that is exactly the pre-GH-4593 node table.
