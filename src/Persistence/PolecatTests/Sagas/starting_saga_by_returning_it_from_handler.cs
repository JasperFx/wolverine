...
public class starting_saga_by_returning_it_from_handler
{
    ...
    public async Task create_sagas_from_a_starting_message()
    {
        ...
        var one = await session.LoadAsync<PcSaga1>(sagaId, TestContext.Current.CancellationToken);
        one.ShouldNotBeNull();

        // The cascading messages should have set this
        one.GotOne.ShouldBeTrue();

        var two = await session.LoadAsync<PcSaga2>(sagaId, TestContext.Current.CancellationToken);
        two.ShouldNotBeNull();
