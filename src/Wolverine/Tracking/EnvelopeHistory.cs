...
{
    // GH-3069/GH-3070, backported from 6.x. Records arrive concurrently -- from parallel transport
    // listener threads when external transports are tracked, and equally from HANDLER-COMPLETION
    // threads on the purely local path via WolverineRuntime.MessageSucceeded -- so every access to
    // _records has to be synchronized. Unsynchronized, a List<T> resize racing a read threw
    // NullReferenceException out of IsComplete(), surfacing as an AggregateException from
    // TrackedSession.AssertNoExceptionsWereThrown() with no hint of the real cause.
    ...
    public IEnumerable<EnvelopeRecord> Records
    {
        get
        {
            // A snapshot, not the live list: handing _records out directly lets a caller enumerate it
            // while a recording thread is adding to it.
            lock (_lock)
            {
