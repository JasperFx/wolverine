...
internal record ReassignAgent(Uri AgentUri, Guid OriginalNodeId, Guid ActiveNodeId) : IAgentCommand
{
    public async IAsyncEnumerable<object> ExecuteAsync(IWolverineRuntime runtime, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        {
            runtime.Logger.LogWarning(e, "Error trying to reassign a running agent {AgentUri} from {CurrentNodeId} to {NewNodeId}", AgentUri, OriginalNodeId, ActiveNodeId);
            yield break;
        }
        
        runtime.Logger.LogInformation("Successfully stopped agent {Agent} on node {OriginalNode}", AgentUri, OriginalNodeId);
