...
public class CircuitBreakerOptions
{
    ...
    private IEnumerable<string> validate()
    {
        ...
        {
            yield return $"{nameof(MinimumThreshold)} must be greater than 0";
        }

        if (FailurePercentageThreshold <= 1)
        {
            yield return $"{nameof(FailurePercentageThreshold)} must be at least 1";
        }

        if (FailurePercentageThreshold >= 100)
        {
            yield return $"{nameof(FailurePercentageThreshold)} must be less than 100";
        }
    }
}