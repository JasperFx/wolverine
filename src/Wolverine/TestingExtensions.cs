...
public static class TestingExtensions
{
    ...
    {
        if (message is Envelope e) return e.Message;
    ...
    private static string toListOfMessages(this IEnumerable<object> messages)
    {
        ...
        if (!actual.Any()) return "[no messages]";
    ...
    {
        return envelopes.FirstOrDefault(x => x.Message is T) ?? throw new WolverineMessageExpectationException($"Could not find an envelope with message type {typeof(T).FullNameInCode()}. The actual messages were {envelopes.toListOfMessages()}", envelopes.resolveMessages());
    }
    ...
    public static T ShouldHaveMessageOfType<T>(this IEnumerable<object> messages)
    {
        ...
        return actual.OfType<T>().FirstOrDefault() ?? throw new WolverineMessageExpectationException($"Should be a message of type {typeof(T).FullNameInCode()}, but actual messages were {actual.toListOfMessages()}", actual);
    }
