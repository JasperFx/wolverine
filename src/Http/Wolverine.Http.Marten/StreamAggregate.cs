...
/// <summary>
/// HTTP endpoint return value that streams the latest projected JSON of an
/// event-sourced aggregate directly from Marten to the <see cref="HttpContext.Response"/>.
/// Uses Marten.AspNetCore's <see cref="QueryableExtensions.WriteLatest{T}(IEventStoreOperations, Guid, HttpContext, string, int)"/>
/// under the hood, which pulls the latest aggregate state from the event store without
...
/// for event-sourced aggregates — Marten rebuilds the latest aggregate state
/// from events before streaming. Use <see cref="StreamOne{T}"/> for regular
/// Marten documents that are stored directly (not event-sourced).
/// </para>
/// </summary>
