...
internal class HttpChainFluentValidationPolicy : IHttpPolicy
{
    ...
    {
        foreach (var chain in chains.Where(x => x.RequestType != null))
        {
   ...
   public void Apply(HttpChain chain, IContainer container)
    {
        ...
        if (registered.Instances.Count() == 1)
        {
            ...
            var method = typeof(FluentValidationHttpExecutor).GetMethod(nameof(FluentValidationHttpExecutor.ExecuteOne))!
        ...
        else if (registered.Instances.Count() > 1)
        {
            ...
            var method = typeof(FluentValidationHttpExecutor).GetMethod(nameof(FluentValidationHttpExecutor.ExecuteMany))!
                .MakeGenericMethod(chain.RequestType);
